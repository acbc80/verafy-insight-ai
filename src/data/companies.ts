...
type DbCompanyRow = {
  id: string; name: string; ticker: string; exchange: string; sector: string;
  country: string; overall_score: number; overall_rating: string;
  e_score: number; s_score: number; g_score: number; summary: string; report_year: number;
};
type DbMetricRow = {
  id: string; company_id: string; name: string; category: string; score: number; rating: string;
  summary: string; detail: string; source_standard: string; source_reference: string;
  source_methodology: string; source_report_page: string | null;
};
type DbCriterionRow = {
  id: number; company_id: string; name: string; score: number; rating: string; summary: string;
};

function mapCompany(row: DbCompanyRow, metrics: DbMetricRow[], criteria: DbCriterionRow[]): Company {
  return {
    id: row.id,
    name: row.name,
    ticker: row.ticker,
    ...
    overallRating: row.overall_rating as ESGRating,
    eScore: row.e_score,
    sScore: row.s_score,
    gScore: row.g_score,
    summary: row.summary,
    ...
    metrics: metrics.map((m) => ({
      id: m.id,
      name: m.name,
      category: m.category as "E" | "S" | "G",
      score: m.score,
      rating: m.rating as ESGRating,
      summary: m.summary,
      detail: m.detail,
      source: {
    ...
    assessmentCriteria: criteria.map((c) => ({
      name: c.name,
      score: c.score,
      rating: c.rating as ESGRating,
      summary: c.summary,
    })),
  };
}
...
export async function fetchCompanies(): Promise<Company[]> {
  const [{ data: rows, error: e1 }, { data: metrics, error: e2 }, { data: criteria, error: e3 }] =
    await Promise.all([
      supabase.from("companies").select("*").order("overall_score", { ascending: false }),
      supabase.from("esg_metrics").select("*"),
      supabase.from("assessment_criteria").select("*"),
    ]);
  if (e1 || e2 || e3) throw e1 ?? e2 ?? e3;
  return (rows as DbCompanyRow[]).map((row) =>
    mapCompany(
      row,
      (metrics as DbMetricRow[]).filter((m) => m.company_id === row.id),
      (criteria as DbCriterionRow[]).filter((c) => c.company_id === row.id),
    )
  );
}

export async function fetchCompany(id: string): Promise<Company | null> {
  const [{ data: row, error: e1 }, { data: metrics, error: e2 }, { data: criteria, error: e3 }] =
    await Promise.all([
      supabase.from("companies").select("*").eq("id", id).single(),
      supabase.from("esg_metrics").select("*").eq("company_id", id),
      supabase.from("assessment_criteria").select("*").eq("company_id", id),
    ]);
  if (e1) return null;
  if (e2 || e3) throw e2 ?? e3;
  return mapCompany(row as DbCompanyRow, metrics as DbMetricRow[], criteria as DbCriterionRow[]);
}
